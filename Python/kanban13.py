...
uploaded_avatar = st.sidebar.file_uploader(
    "🖼️ Cambiar avatar (seleccionar la tacha para salir de modo selección de avatar)",
    type=["png", "jpg", "jpeg"]
)
...
if uploaded_avatar:
    ...
    new_path = os.path.join(
        AVATAR_DIR,
        f"{st.session_state.username}.{ext}"
    )
...
if st.session_state.role in ("admin", "edit"):
    ...
    estimated_time = st.sidebar.text_input("⏱️ Tiempo estimado")
    ...
    with cols[col_idx]:
        ...
        for task in list(st.session_state.board[col_name]):
            ...
            st.markdown(
                ...
                👤 {task['owner']}<br>
                🧑‍💻 {task.get('created_by','')}<br>
                ⏱️ Estimado: {task.get('estimated_time','')}
                </div>
                """,
                unsafe_allow_html=True
            )
